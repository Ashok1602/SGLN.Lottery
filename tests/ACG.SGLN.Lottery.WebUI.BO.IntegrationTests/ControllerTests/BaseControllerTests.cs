...
namespace ACG.SGLN.Lottery.WebUI.BO.IntegrationTests.ControllerTests
{
        ...
        where TVm : class
    {
        ...
        protected virtual async Task<TContentType> GetResponseContent<TContentType>(HttpResponseMessage httpResponse)
        {
            var content = await httpResponse.Content.ReadAsStringAsync();
            Console.WriteLine(content);
            return JsonConvert.DeserializeObject<TContentType>(content);
        }
        ...
        protected virtual async Task<TEntity> TryAdd(object dto, HttpStatusCode? statusCode = null, string requestUri = null)
        {
            var httpResponse = await HttpClient.PostAsync(string.IsNullOrEmpty(requestUri) ? GetRequestUri() : requestUri, CreatePostBody(dto));
            if (statusCode == null)
