...
const RequestReport = (props) => {
  ...
  const [lastStatus, updateLastStatus] = useState(null);
  ...
  const nextProps = useSelector((state) => ({
    report: state.ReportModule.ratioReportData,
  ...
  useEffect(() => {
    ...
    if (nextProps.report && nextProps.report.data) {
      saveAs(
        ...
        `${TRANSLATE.t('REPORT.RATIO_REQESTS')}.${isGenerate === 'Excel' ? 'xlsx' : 'pdf'}`
      );
      reset();
    }
  ...
  const onSubmit = (formData) => {
    ...
    dispatch(generateRatioReports(formData));
    ...
    <div className='mx-3'>
      <AvForm className='form-horizontal' noValidate>
        <Card className='top-margin'>
          <CardHeader>
            <h1>{TRANSLATE.t('REPORT.RATIO_REQESTS')}</h1>
          </CardHeader>
          <CardBody>
            <Row>
              {showSelect && (
            ...
            <Row className='mt-2'>
              <Col>
                ...
                <Field
                  ...
                  onChange={e => updateLastStatus(e.target.value)}
                />
              </Col>
              <Col>
        ...
        >
          <Col xl='2'>
            <Button
              ...
              disabled={lastStatus && lastStatus !== "0" ? false : true}
            >
