...
import { renderOptions, getRequestNatureTypes } from '../../helpers';

const TimeProcessRequestReport = (props) => {
  const { reset } = props;
  ...
  const nextProps = useSelector((state) => ({
    report: state.ReportModule.timeReportdata,
  ...
  useEffect(() => {
    ...
    if (nextProps.report && nextProps.report.data) {
      saveAs(
        ...
        `${TRANSLATE.t('REPORT.TIME_PROCESSING_REQUETS')}.${isGenerate === 'Excel' ? 'xlsx' : 'pdf'}`
      );
      reset();
    }
  ...
  const onSubmit = (formData) => {
    ...
    dispatch(generateTimeRequestReport(formData));
    ...
    <div className='mx-3'>
      <AvForm className='form-horizontal' noValidate>
        <Card className='top-margin'>
          <CardHeader>
            <h1>{TRANSLATE.t('REPORT.TIME_PROCESSING_REQUETS')}</h1>
          </CardHeader>
          <CardBody>
            <Row>
            {showSelect &&  <Col>
...
TimeProcessRequestReport.propTypes = {
  handleSubmit: PropTypes.func.isRequired,
  reset: PropTypes.func.isRequired,
};

export default reduxForm({
  form: 'TimeProcessingRequestsForm',
})(TimeProcessRequestReport);
