...
namespace ACG.SGLN.Lottery.Application.Reporting.Queries.GetTrainingsByRetailerReport
{
    public class GetTrainingsByRetailerReportQuery : IRequest<byte[]>
    {
        public DateTime? FromDate { get; set; }
        public DateTime? ToDate { get; set; }
        public TrainingsByRetailerReportCriterea Criterea { get; set; } = new TrainingsByRetailerReportCriterea();
        public DocumentFormat Format { get; set; }
    }

    public class GetTrainingsByRetailerReportQueryHandler : IRequestHandler<GetTrainingsByRetailerReportQuery, byte[]>,
        IApplicationRequestHandler<GetTrainingsByRetailerReportQuery, byte[]>
    {
        private readonly IApplicationDbContext _context;
        private readonly IPdfPrintService _pdfPrintService;
        private readonly IExcelPrintService _excelPrintService;
        public GetTrainingsByRetailerReportQueryHandler(IPdfPrintService pdfPrintService, IApplicationDbContext context, IExcelPrintService excelPrintService)
        {
            _context = context;
        ...
        public async Task<byte[]> Handle(GetTrainingsByRetailerReportQuery request, CancellationToken cancellationToken)
        {
            ...
            List<TrainingsByRetailerReportDto> data = await _context.ApplySpecification
                (new TrainingsByRetailerReportSearchSpecification(request))
                .OrderByDescending(vp => vp.Created)
                .Select(vp => GetReportItem(vp))
                .ToListAsync();

            if (request.Format == DocumentFormat.Pdf)
                return await _pdfPrintService.GeneratePdf<List<TrainingsByRetailerReportDto>>(TemplatesNames.PDFs.TrainingsByRetailer, data, true);
            else
                return await _excelPrintService.GenerateExcel<TrainingsByRetailerReportDto>(data, new List<string> {
                                "Civilité","Prénom","Nom","Ville","Téléphone","Formation","Module","Date début formation","Date fin formation","Nombre d'heures","Résultat"
                }, "Rapport formation par détaillant");
        }

        private static TrainingsByRetailerReportDto GetReportItem(RetailerTraining rt)
        {
            ...
            return new TrainingsByRetailerReportDto
            {
